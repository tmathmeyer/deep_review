...
class Reviewer(ABC):
    ...
    async def deduce_more_context(self, change_info: ChangeInfo, output_dir: Path, vync_app: Vync) -> None:
        pass

    async def perform_analysis(self, change_info: ChangeInfo, output_dir: Path, vync_app: Vync) -> dict:
        agents_dir = self.get_reviewer_agents_dir()
        return await analyze_context(output_dir, self.gemini_client, self.model_name, agents_dir)

    @abstractmethod
    def get_reviewer_agents_dir(self) -> Path:
        pass

    async def run_review_agents(self, change_info: ChangeInfo, output_dir: Path, vync_app: Vync) -> None:
        agents_dir = self.get_reviewer_agents_dir()
        await run_review(output_dir, self.gemini_client, self.model_name, agents_dir, vync_app)

    async def coalesce_reviews(self, change_info: ChangeInfo, output_dir: Path, vync_app: Vync) -> str:
