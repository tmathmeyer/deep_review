...
class LocalReviewer(Reviewer):
    ...
    async def _run_git_command(self, *args: str, timeout: float = 30.0, return_bytes: bool = False):
        proc = None
        try:
    ...
    async def _get_base_commit(self) -> str:
        ...
        for candidate in candidates:
            try:
                commit = await self._run_git_command("merge-base", "HEAD", candidate, timeout=5.0)
                if commit:
                    return commit
            except FileNotFoundError:
                print("\n[Error] git executable not found. Please ensure git is installed.", file=sys.stderr)
                raise
        ...
        try:
            roots_out = await self._run_git_command("rev-list", "--max-parents=0", "HEAD", timeout=5.0)
        ...
        raise RuntimeError("Failed to determine a base commit for local changes. Make sure you are in a valid git repository with at least one commit.")
    ...
    async def fetch_change(self, target: str, output_dir: Path, vync_app: Vync) -> ChangeInfo:
        ...
        try:
            ...
            repo_root_str = await self._run_git_command("rev-parse", "--show-toplevel", timeout=5.0)
        ...
        async def _fetch_files() -> None:
            ...
            try:
                if not is_specific_commit:
                    ...
                    if proc.returncode != 0:
                        raise RuntimeError(f"pipeline failed: {stderr.decode(errors='replace')}")
                else:
                    files_out = await self._run_git_command("diff", "--name-only", "-z", "--diff-filter=d", base_commit, head_commit, return_bytes=True)
                    if not files_out:
                        return
                    ...
                    for i in range(0, len(files), chunk_size):
                        ...
                        _, stderr = await asyncio.wait_for(proc.communicate(), timeout=60.0)
                        if proc.returncode != 0:
                            raise RuntimeError(f"git archive pipeline failed: {stderr.decode(errors='replace')}")
            except asyncio.TimeoutError:
